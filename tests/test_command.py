...
class TestCommandRegistry:
    def test_basic_registration_and_lookup(self):
        ...
        a = Command(key="a", name="A", description="Desc A")
        b = Command(key="b", name="B", description="Desc B")
    ...
    def test_all_returns_copy(self):
        ...
        a = Command(key="a", name="A", description="...")
        b = Command(key="b", name="B", description="...")
...
class TestDuplicateKeys:
    def test_duplicate_key_raises(self):
        ...
        Command(key="dup", name="First", description="...")
        with pytest.raises(ValueError) as exc:
            Command(key="dup", name="Second", description="...")
    ...
    def test_eq_but_duplicate_key_blocked(self):
        ...
        _first = Command(key="dup", name="D", description="d")
        with pytest.raises(ValueError):
            Command(key="dup", name="D", description="d")


class TestImmutability:
    def test_command_is_immutable(self):
        # Command instances should be frozen dataclasses
        cmd = Command(key="x", name="X", description="desc")
